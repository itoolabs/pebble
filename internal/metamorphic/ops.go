...
// closeOp models a Batch.Close operation.
type closeOp struct {
	objID objID
}
...
func (o *ingestOp) build(t *test, h *history, b *pebble.Batch, i int) (string, error) {
	path := fmt.Sprintf("tmp/ext%d", i)
	f, err := t.opts.FS.Create(path)
	if err != nil {
		return "", err
	}

	iter, rangeDelIter := batch.Sort(b)
	defer func() {
		if iter != nil {
			iter.Close()
		}
	...
	w := sstable.NewWriter(f, t.opts, t.opts.Level(0))
	...
	for key, value := iter.First(); key != nil; key, value = iter.Next() {
		...
		// NB: We don't have to copy the key or value to reading from the batch.
